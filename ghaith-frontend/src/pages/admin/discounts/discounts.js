...
export function initDiscounts() {
  ...
  const openEditor = item => { editing = item; form.reset(); name.value = item?.name || ""; value.value = item?.discount_percent ?? 0; error.textContent = ""; modalTitle.textContent = item ? "تعديل الخصم" : "إضافة فئة عميل"; saveButton.textContent = item ? "حفظ التعديل" : "إضافة الفئة"; setOpen(true); requestAnimationFrame(() => name.focus()); };
  const onTable = event => { const button = event.target.closest(".discount-edit"); if (!button) return; const item = items.find(entry => String(entry.id) === button.closest("tr").dataset.id); if (item) openEditor(item); };
  const submit = async event => { event.preventDefault(); const amount = Number(value.value), typeName = name.value.trim(); if (!typeName || !Number.isFinite(amount) || amount < 0 || amount > 100) { error.textContent = "أدخل اسمًا ونسبة بين 0 و100"; return; } saveButton.disabled = true; try { if (editing) await api.patch(`/api/v1/admin/customer-types/${encodeURIComponent(editing.id)}`, { name: typeName, discount_percent: amount }); else await api.post("/api/v1/admin/customer-types", { name: typeName, discount_percent: amount }); setOpen(false); success.hidden = false; await load(); } catch (e) { error.textContent = e.status === 405 || e.status === 404 ? "الخادم الحالي لا يدعم إنشاء فئة خصم جديدة بعد." : e.message; } finally { saveButton.disabled = false; } };
