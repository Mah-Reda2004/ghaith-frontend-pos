...
function addApiUi() {
  ...
  document.querySelector(".suppliers-table thead").innerHTML = "<tr><th>اسم المورد</th><th>رقم الهاتف</th><th>العنوان</th><th>المبلغ المدفوع</th><th>المبلغ المتبقي</th><th>الحالة</th><th>الإجراءات</th></tr>";
  ...
  document.querySelector(".suppliers-page").insertAdjacentHTML("beforeend", `
    ...
    <div class="modal-overlay suppliers-modal" id="supplierDeactivateModal" hidden><section class="modal suppliers-confirm" role="alertdialog" aria-modal="true" aria-labelledby="supplierDeactivateTitle"><span class="suppliers-confirm__icon">!</span><h2 id="supplierDeactivateTitle">تعطيل المورد؟</h2><p>سيتم تعطيل <b id="supplierDeactivateName"></b> ويمكن إعادة تفعيله لاحقًا من التعديل.</p><div><button class="btn btn-outline" id="cancelSupplierDeactivate" type="button">إلغاء</button><button class="btn suppliers-confirm__submit" id="confirmSupplierDeactivate" type="button">تعطيل المورد</button></div></section></div>`);
}
...
function normalizeSupplier(item) { return { id: item.id, name: item.name || "—", phone: item.phone || "—", address: item.address || "", notes: item.notes || "", status: item.status || "active", createdAt: item.created_at }; }
...
function renderSuppliers(elements) {
  ...
  elements.tableBody.innerHTML = rows.map(supplier => { const balance = supplierBalances.get(String(supplier.id)); return `<tr data-supplier-id="${escapeHtml(String(supplier.id))}"><td><button class="supplier-name-button" type="button" data-action="view">${escapeHtml(supplier.name)}</button></td><td class="num" dir="ltr">${escapeHtml(supplier.phone)}</td><td>${escapeHtml(supplier.address || "—")}</td><td class="num supplier-paid">${balance ? money(balance.paid) : "…"}</td><td class="num supplier-balance">${balance ? money(balance.remaining) : "…"}</td><td><span class="suppliers-status suppliers-status--${supplier.status}">${supplier.status === "active" ? "نشط" : "غير نشط"}</span></td><td><div class="suppliers-actions"><button class="suppliers-action suppliers-action--edit" type="button" data-action="edit" aria-label="تعديل"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9M16.5 3.5a2.1 2.1 0 0 1 3 3L8 18l-4 1 1-4Z"/></svg></button><button class="suppliers-action suppliers-action--delete" type="button" data-action="deactivate" aria-label="تعطيل"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="7" r="4"/><path d="M2 21a7 7 0 0 1 14 0M17 11h5"/></svg></button></div></td></tr>`; }).join("");
...
async function saveSupplier(elements) {
  ...
  try { if (id) await api.patch(`/api/v1/admin/suppliers/${encodeURIComponent(id)}`, payload); else await api.post("/api/v1/admin/suppliers", payload); closeSupplierModal(elements); showToast(elements, id ? "تم تحديث المورد" : "تمت إضافة المورد"); await loadSuppliers(elements); } catch (error) { showToast(elements, error.message, true); }
}

async function showSupplierDetail(elements, supplier) {
  activeSupplier = supplier; elements.detailName.textContent = supplier.name; elements.detailType.textContent = supplier.address || "لا يوجد عنوان"; elements.detailStatus.className = `suppliers-status suppliers-status--${supplier.status}`; elements.detailStatus.textContent = supplier.status === "active" ? "مورد نشط" : "مورد غير نشط"; elements.detailId.textContent = supplier.id; elements.detailDate.textContent = dateLabel(supplier.createdAt); elements.detailNotes.textContent = supplier.notes || "لا توجد ملاحظات"; elements.invoicesBody.innerHTML = '<tr><td colspan="6">جاري تحميل الفواتير...</td></tr>'; setView(elements, "detail");
  try { const invoices = await fetchAll(`/api/v1/admin/suppliers/${encodeURIComponent(supplier.id)}/purchase-invoices`); const total = invoices.reduce((sum, invoice) => sum + Number(invoice.total_amount || 0), 0), paid = invoices.reduce((sum, invoice) => sum + Number(invoice.paid_amount || 0), 0); elements.detailTotal.textContent = `EGP ${money(total)}`; elements.detailPaid.textContent = `EGP ${money(paid)}`; elements.detailBalance.textContent = `EGP ${money(total - paid)}`; elements.invoicesBody.innerHTML = invoices.map(invoice => `<tr class="purchase-invoice-row" data-invoice-id="${escapeHtml(String(invoice.id))}" tabindex="0"><td>${escapeHtml(invoice.invoice_number)}</td><td>${invoice.payment_method === "cash" ? "نقدي" : "آجل"}</td><td class="num">${money(invoice.total_amount)}</td><td class="num supplier-paid">${money(invoice.paid_amount)}</td><td class="num supplier-balance">${money(invoice.remaining_amount)}</td><td>${escapeHtml(invoice.status)}</td></tr>`).join("") || '<tr><td colspan="6">لا توجد فواتير شراء</td></tr>'; } catch (error) { elements.invoicesBody.innerHTML = `<tr><td colspan="6">${escapeHtml(error.message)}</td></tr>`; }
}
...
function addPurchaseLine(elements) { const row = document.createElement("div"); row.className = "purchase-line"; row.innerHTML = `<select class="select purchase-product"><option value="">اختر المنتج...</option>${products.map(product => `<option value="${escapeHtml(String(product.id))}" data-price="${product.price}">${escapeHtml(product.name)}</option>`).join("")}</select><input class="input num purchase-qty" type="number" min="1" value="1" aria-label="الكمية"><input class="input num purchase-price" type="number" min="0" step="0.01" value="0" aria-label="سعر الوحدة"><button class="suppliers-action suppliers-action--delete purchase-remove" type="button" aria-label="حذف البند">×</button>`; elements.purchaseLines.append(row); }
async function openPurchase(elements) { if (!activeSupplier) return; try { await loadProductOptions(); elements.purchaseForm.reset(); elements.purchaseLines.innerHTML = ""; document.getElementById("purchaseSupplierName").textContent = activeSupplier.name; addPurchaseLine(elements); elements.purchaseModal.hidden = false; } catch (error) { showToast(elements, error.message, true); } }
...
async function savePurchase(elements) {
  ...
  try { for (const row of rows) { const productId = row.querySelector(".purchase-product").value; if (!productId) throw new Error("اختر منتجًا لكل بند."); const detail = await api.get(`/api/v1/products/${encodeURIComponent(productId)}`); const variant = detail.product_variants?.[0]; if (!variant) throw new Error("المنتج المحدد لا يحتوي على variant متاح."); items.push({ variant_id: variant.id, quantity: Math.max(1, Number(row.querySelector(".purchase-qty").value) || 1), unit_price: Math.max(0, Number(row.querySelector(".purchase-price").value) || 0), expected_version: variant.version }); }
    if (!elements.invoiceNumber.value.trim()) throw new Error("رقم الفاتورة مطلوب."); const deferred = elements.paymentMethod.value === "deferred"; elements.savePurchase.disabled = true; await api.post(`/api/v1/admin/suppliers/${encodeURIComponent(activeSupplier.id)}/purchase-invoices`, { invoice_number: elements.invoiceNumber.value.trim(), payment_method: elements.paymentMethod.value, paid_amount: Math.max(0, Number(elements.paidAmount.value) || 0), due_date: deferred ? elements.dueDate.value || null : null, notes: elements.purchaseNotes.value.trim() || null, items }, { headers: { "Idempotency-Key": crypto.randomUUID() } }); supplierBalances.delete(String(activeSupplier.id)); closePurchase(elements); showToast(elements, "تمت إضافة فاتورة الشراء"); await showSupplierDetail(elements, activeSupplier); } catch (error) { showToast(elements, error.message, true); } finally { elements.savePurchase.disabled = false; }
}
...
export function initSuppliers() {
  ...
  const tableClick = async event => { const action = event.target.closest("[data-action]"), row = action?.closest("[data-supplier-id]"); if (!action || !row) return; const supplier = suppliers.find(item => String(item.id) === row.dataset.supplierId); if (!supplier) return; if (action.dataset.action === "view") await showSupplierDetail(elements, supplier); if (action.dataset.action === "edit") openSupplierModal(elements, supplier); if (action.dataset.action === "deactivate" && supplier.status === "active" && await confirmDeactivation(elements, supplier)) { try { await api.patch(`/api/v1/admin/suppliers/${encodeURIComponent(supplier.id)}`, { status: "inactive" }); showToast(elements, "تم تعطيل المورد"); await loadSuppliers(elements); } catch (error) { showToast(elements, error.message, true); } } };
  ...
  return () => { requestSequence += 1; refresh.cancel?.(); elements.tableBody.removeEventListener("click", tableClick); };
}
