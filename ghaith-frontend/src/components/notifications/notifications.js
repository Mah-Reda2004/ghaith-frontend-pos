...
export function initNotificationCenter(){
  ...
  const unlockAudio=event=>{if(event&&!event.isTrusted)return;if(!audioContext){const Context=window.AudioContext||window.webkitAudioContext;if(Context)audioContext=new Context()}if(audioContext?.state==="suspended")audioContext.resume().catch(()=>{})};document.addEventListener("pointerdown",unlockAudio,{once:true});
