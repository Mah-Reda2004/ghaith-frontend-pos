...
(async () => {
  ...
  try {
    ...
    const pageErrors = [];
    let checkoutBody; let checkoutAttempts = 0; let productLoads = 0;
    ...
    assert.equal(await card.locator('.product-card__details span', { hasText: 'L' }).count(), 1);
    ...
    assert.equal(await page.locator('#productGrid .product-card').count(), 1);
    await card.click();
    assert.match(await page.locator('#variantPickerGrid .variant-picker__columns').textContent(), /المقاس/);
    assert.match(await page.locator('#variantPickerGrid .variant-picker__columns').textContent(), /اللون/);
    assert.match(await page.locator('#variantPickerGrid .variant-option').first().textContent(), /L/);
    assert.match(await page.locator('#variantPickerGrid .variant-option').first().textContent(), /أبيض/);
    const optionBoxes = await page.locator('#variantPickerGrid .variant-option').evaluateAll(nodes => nodes.map(node => node.getBoundingClientRect().toJSON()));
    assert.equal(optionBoxes[0].x, optionBoxes[1].x);
    assert.ok(optionBoxes[1].y > optionBoxes[0].y);
    ...
    assert.equal(await page.locator('#variantPickerGrid').evaluate(node => node.scrollWidth > node.clientWidth), true);
    await page.setViewportSize({ width: 1440, height: 1000 });
    await page.locator('#variantPickerGrid [data-variant-id="cccccccc-cccc-4ccc-8ccc-cccccccccccc"]').click();
    assert.match(await page.locator('#cartList').textContent(), /أبيض\s*·\s*L/);
    await card.click();
    await page.locator('#variantPickerGrid [data-variant-id="cccccccc-cccc-4ccc-8ccc-cccccccccccc"]').click();
