...
(async () => {
  ...
  try {
    ...
    await page.route('https://test-3f530955.fastapicloud.dev/**', route => {
      ...
      if (path === '/api/v1/products/search') return route.fulfill({ json: { items: [{ id: 'p3', name_ar: 'عطر جديد API', sale_price: 125, product_variants: [{ id: 'abababab-abab-4bab-8bab-abababababab', sku: 'NEW-PERFUME', stock_qty: 3, version: 1 }] }] } });
      ...
      if (path === '/api/v1/pos/sales-users') return route.fulfill({ json: { items: [{ id: '77777777-7777-4777-8777-777777777777', name: 'سيلز النظام', is_active: true }] } });
    ...
    await page.getByText('عطر جديد API', { exact: true }).waitFor();
    assert.equal(await page.getByText('ثوب API', { exact: true }).count(), 0);
    await page.getByText('عطر جديد API', { exact: true }).click();
